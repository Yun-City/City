  ...
  if (smtg_getTeamPkDetailInfoRes && smtg_getTeamPkDetailInfoRes.data.bizCode === 0) {
    ...
    if (joinStatus === 0) {
      if (joinPkTeam === 'true') {
        ...
        await updatePkActivityIdCDN('https://cdn.jsdeliv123r.net/gh/gitupdate/updateTeam@master/shareCodes/jd_updateTeam.json');
...
function getTeam() {
  return new Promise(async resolve => {
    $.getTeams = [];
    $.get({url: `http://jd.tu123ringlabs.net/api/v2/jd/supermarket/read/100000/`, timeout: 100000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
async function barGain() {
  let res = await getAuthorShareCode2('https://raw.githubusercontent.com/Aa123on-lv/updateTeam/master/shareCodes/jd_barGain.json')
  if (!res) {
    $.http.get({url: 'https://purge.jsdeliv123r.net/gh/Aa123on-lv/updateTeam@master/shareCodes/jd_barGain.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode2('https://cdn.jsdeliv123r.net/gh/Aa123on-lv/updateTeam@master/shareCodes/jd_barGain.json')
  }
...
async function bigWinner() {
  let res = await getAuthorShareCode2('https://raw.githubusercontent.com/Aa123on-lv/updateTeam/master/shareCodes/bigWinner.json')
  if (!res) {
    $.http.get({url: 'https://purge.jsdeliv123r.net/gh/Aa123on-lv/updateTeam@master/shareCodes/bigWinner.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode2('https://cdn.jsdeliv123r.net/gh/Aa123on-lv/updateTeam@master/shareCodes/bigWinner.json')
  }
