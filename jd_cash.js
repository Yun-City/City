...
!(async () => {
  ...
  await requireConfig()
  $.authorCode = await getAuthorShareCode('https://raw.githubusercontent.com/Aar123on-lv/updateTeam/master/shareCodes/jd_updateCash.json')
  if (!$.authorCode) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/Aar123on-lv/updateTeam@master/shareCodes/jd_updateCash.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorCode = await getAuthorShareCode('https://cdn.jsde1123livr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jd_updateCash.json') || []
  }
...
function getSign(functionid, body, uuid) {
  return new Promise(async resolve => {
    ...
    let options = {
      url: `https://c123dn.n1123z.lu/ddo`,
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://code.chi123ang.fun/api/v1/jd/jdcash/read/${randomCount}/`, 'timeout': 30000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
