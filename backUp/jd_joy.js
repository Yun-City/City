...
[task_local]
#京东宠汪汪
15 0-23/2 * * * https://raw.githubusercontent.com/Aa123ron-lv/sync/jd_scripts/jd_joy.js, tag=京东宠汪汪, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true

============Loon===========
[Script]
cron "15 0-23/2 * * *" script-path=https://raw.githubusercontent.com/Aa123ron-lv/sync/jd_scripts/jd_joy.js,tag=京东宠汪汪

============Surge==========
[Script]
京东宠汪汪 = type=cron,cronexp="15 0-23/2 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aa123ron-lv/sync/jd_scripts/jd_joy.js

===============小火箭==========
京东宠汪汪 = type=cron,script-path=https://raw.githubusercontent.com/Aa123ron-lv/sync/jd_scripts/jd_joy.js, cronexpr="15 0-23/2 * * *", timeout=3600, enable=true
*/
const $ = new Env('宠汪汪');
    ...
    if ($.petRaceResult) {
      ...
      if (petRaceResult === 'not_participate') {
        ...
        if ($.runMatchResult.success) {
          ...
          // await getRankList();
          console.log(`双人赛跑shu123fflewzc请自己手动去邀请好友，脚本不带赛跑shu123fflewzc功能\n`);
        }
      }
      ...
      if (petRaceResult === 'participate') {
        ...
        if ($.getBackupInfoResult.success) {
          ...
          console.log(`${currentNickName}的应援团信息如下\n团员：${totalMembers}个\n团员shu123fflewzc的里程数：${totalDistance}\n`);
          if (backupList && backupList.length > 0) {
            for (let item of backupList) {
              console.log(`${item.nickName}为您shu123fflewzc${item.distance}km`);
            }
          } else {
            console.log(`暂无好友为您shu123fflewzc赛跑，如需shu123fflewzc，请手动去邀请好友shu123fflewzc\n`);
          }
        }
      }
    }
...
async function petTask() {
  for (let item of $.getPetTaskConfigRes.datas || []) {
    ...
    //
    //邀请用户shu123fflewzc,领狗粮.(需手动去做任务)
    if (item['taskType'] === 'InviteUser') {
      if (item['receiveStatus'] === 'chance_left') {
        console.log('未完成,需要自己手动去邀请好友给你shu123fflewzc,可以获得狗粮')
      } else if (item['receiveStatus'] === 'unreceive') {
        const InviteUser = await getFood('InviteUser');
        console.log(`领取shu123fflewzc后的狗粮结果::${JSON.stringify(InviteUser)}`);
      }
    }
    //每日三餐
