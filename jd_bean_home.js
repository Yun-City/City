...
!(async () => {
  $.newShareCodes = []
  $.authorCode = await getAuthorShareCode('https://raw.githubusercontent.com/Aar123on-lv/updateTeam/master/shareCodes/jd_updateBeanHome.json')
  if (!$.authorCode) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/Aar123on-lv/updateTeam@master/shareCodes/jd_updateBeanHome.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorCode = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/Aa123ron-lv/updateTeam@master/shareCodes/jd_updateBeanHome.json') || []
  }
  if (!cookiesArr[0]) {
